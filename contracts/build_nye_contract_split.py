#!/usr/bin/env python3
"""NYE 2026 special-event agreement — SPLIT into two per-venue contracts (Norbert 5773).
Combined 4-DJ contract (build_nye_contract.py) split into:
  - LDK  : Zara Gift + Joyyly
  - NOBU : Funktastic + Riot Huntt
Each = 2 DJs @ 55,000/night standby (Sub 110,000 -> Net 114,400). Venue party, terms,
brand and structure are verbatim from build_nye_contract.py; only the DJ line-up, the
venue name in the recitals, and the fee (4->2 DJs) change. 20% deposit / 80% balance.
"""
import json, base64
...
# ---- Fee figures (2 DJs @ 55,000 standby) — asserted ----
DJS=2; RATE=55000
SUB=DJS*RATE                    # 110,000
VAT=round(SUB*0.07)             # 7,700
GRAND=SUB+VAT                   # 117,700
WHT=round(SUB*0.03)             # 3,300
NET=GRAND-WHT                   # 114,400
assert (SUB,VAT,GRAND,WHT,NET)==(110000,7700,117700,3300,114400), (SUB,VAT,GRAND,WHT,NET)
DEP_GRAND=round(GRAND*0.20)     # 23,540
BAL_GRAND=GRAND-DEP_GRAND       # 94,160
DEP_WHT=round(WHT*0.20)         # 660
BAL_WHT=WHT-DEP_WHT             # 2,640
DEP_NET=DEP_GRAND-DEP_WHT       # 22,880
BAL_NET=BAL_GRAND-BAL_WHT       # 91,520
assert (DEP_GRAND,BAL_GRAND)==(23540,94160), (DEP_GRAND,BAL_GRAND)
assert DEP_NET+BAL_NET==NET, (DEP_NET,BAL_NET,NET)
NET_WORDS="ONE HUNDRED FOURTEEN THOUSAND FOUR HUNDRED BAHT"

def build(key, venue_name, dj_list):
    fee_box=f"""
    <div class="feebox">
      <div class="fl"><div class="d">DJ Performance (standby) &mdash; two (2) DJs
          <small>&mdash; {dj_list} &middot; New Year&rsquo;s Eve, 31 December 2026 &middot; {venue_name} &middot; on standby all night</small></div>
        <div class="n">2 &times; THB 55,000 / night = <b>THB {thb(SUB)}</b></div></div>
    ...
    <p style="text-align:center;color:{MUTED};font-size:9.5px;margin-top:2px">{NET_WORDS}</p>"""
    ...
    HTML=f"""<!DOCTYPE html><html><head><meta charset="utf-8"><style>{CSS}</style></head><body>
      ...
      <section class="hours"><h2>Hours of Work</h2>
        <p>This engagement covers the New Year&rsquo;s Eve programme on <span class="b">31 December 2026</span>. <span class="b">Two (2) DJs &mdash; {dj_list}</span> &mdash; shall be on standby for the full night at {venue_name}, performing as directed by the Venue to maintain the energy of the evening throughout.</p>
      </section>
    ...
    path=f"{OUT}/Bright-Ears-NYE-2026-{key}-Agreement.html"
    open(path,"w").write(HTML)
    print(f"built {key}: {venue_name} | {dj_list} | Sub {SUB} VAT {VAT} Grand {GRAND} WHT {WHT} Net {NET}")

build("LDK",  "Le Du Kaan",   "Zara Gift and Joyyly")
build("NOBU", "NOBU Bangkok", "Funktastic and Riot Huntt")
